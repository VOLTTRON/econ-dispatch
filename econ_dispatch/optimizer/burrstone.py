...
def build_problem(forecast, parameters={}):
    ...
    demand_surcharge = VariableGroup("demand_surcharge", indexes=(facility_names,), lower_bound_func=constant_zero)
    ...
    def facilitydemand0(index):
        # [i=1:N_facility]
        i = index[0]
        return demand_surcharge[i] >= facility_para[i].import_peak_on_startup
    add_constraint("facilitydemand0", index_facility_no_hour, facilitydemand0)

    def facilitydemand1(index):
        # [i=1:N_facility,t=1:H_t]
        i, t = index
        return demand_surcharge[i] >= facility_im[i,t]*parasys['peak_hours'][t]
    ...
    def DemandChargeCosts(index):
        i = index[0]
        return demand_charge_costs[i] == demand_surcharge[i]*facility_para[i].demand_charge
