...
def getDesiccantCoeffs(TrainingData,h_oa_min):
    ...
    Time = TrainingData['Date/Time'].values #timestamp
    ...
    T_CA=TrainingData['T_CA'].values # Conditioned temperature of supply air at the outlet of the heat recovery coil
    CFM_OA=TrainingData['CFM_OA'].values #Volumetric flow rate of outdor air in cubic feet per minute (cfm)
    RH_CA=TrainingData['RH_CA'].values # Conditioned relative humidity of supply air at the outlet of the heat recovery coil
    T_hw=TrainingData['T_HW'].values # hot water inlet temperature to regeneration coil
    Fan=TrainingData['FanStatus'].values # Fan status (1=ON, 0=OFF)
    ...
    RegressionArray= [T_HW_fan,h_OA_fan,h_OA2_fan,CFM_OA_fan] #regression variables are hot water inlet temperature, outdoor air enthalpy, outdoor air enthalpy squared and CFM of outdoor air
    y = delta_h
    x = RegressionArray
