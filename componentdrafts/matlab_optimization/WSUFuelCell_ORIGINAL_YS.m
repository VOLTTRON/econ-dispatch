...
for i = 2
    ...
    [FuelFlow,ExhaustFlow,ExhaustTemperature,NetEfficiency] = ...
        FuelCell_Operate(Gen.Power(Gen.Valid),
    Gen.AmbTemperature(Gen.Valid),
    Gen.Start(Gen.Valid),
    Gen.Hours(Gen.Valid),
    Coef);
    figure(i)
    Xdata = Gen.Power(Gen.Valid);
    [Xdata, idx] = sort(Xdata);
    Ydata = FuelFlow(idx)*171.11; % fuel: kg/s -> mmBtu/hr
    
    N = length(Xdata);
    xmin_Turbine = min(Xdata);
    xmax_Turbine = max(Xdata);
    n1 = find(Xdata<=xmin_Turbine(1)+(xmax_Turbine(1)-xmin_Turbine(1))*0.3,1,'last');
    n2 = find(Xdata<=xmin_Turbine(1)+(xmax_Turbine(1)-xmin_Turbine(1))*0.6,1,'last');
    regX = [ones(n2-n1+1,1), Xdata(n1:n2)];
    m = regress(Ydata(n1:n2), regX);
    m_Turbine = m;    
    ...
    hold on
    x = linspace(xmin_Turbine(1)+(xmax_Turbine(1)-xmin_Turbine(1))*0.3, xmax_Turbine(1), 100);
    y = m_Turbine(1,1)+m_Turbine(2,1)*x;
    plot(x,y,'r','LineWidth',3)
    hold off
    
    save('FuelCellPara.mat','xmin_Turbine','xmax_Turbine','m_Turbine');
...
%{ 
    ...
    figure(10+i)
    scatter(Gen.Power(Gen.Valid),Gen.Eff(Gen.Valid)*100.*Gen.Power(Gen.Valid),'.')
